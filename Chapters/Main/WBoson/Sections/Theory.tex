...
where $f_{L}$ correspond to left-handed fermions and $\bar{f}_{R}$ represents right-handed anti-fermions. Thus, {\PW} bosons only couple to right-handed anti-fermions and left-handed fermions organized in pairs of lepton-neutrino or quark-antiquark, where the electric charge of the particles differs by one unit. Since the top-quark mass (\SI{178}{\GeV}) is larger than the \Wb-boson mass (\SI{80}{\GeV}), the \Wb boson can not decay to a virtual top quark. \fig{dia:WDecays} shows the possible decays of weak bosons to fermions. The measured values of the mass, width and couplings of weak vector bosons are summarized in \tab{tab:ElectroweakParameters}.
